...
@objc open class ACFloatingTextfield: UITextField {
    ...
    @IBInspectable open var errorheight : Bool = false
    /// Disable Floating Label when true.
    
    @IBInspectable open var disableFloatingLabel : Bool {
        get {
            return labelPlaceholder?.isHidden ?? true
        }
        set {
            labelPlaceholder?.isHidden = newValue
        }
    }
    ...
    public func showOrHidePlaceHolder(isshow: Bool) {
        print("is hide place holder %@", isshow )
        self.labelPlaceholder?.isHidden = isshow
    }
...
fileprivate extension ACFloatingTextfield {
    ...
    func addFloatingLabel(){
        ...
        labelPlaceholder?.font = UIFont.init(name: (self.font?.fontName ?? "Roboto-Medium")!, size: 15)
        ...
        placeholderLabelHeight = NSLayoutConstraint.init(item: labelPlaceholder!, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: 18)
    ...
    func addErrorPlaceholderLabel() -> Void {
        ...
        labelErrorPlaceholder?.numberOfLines = 2
        labelErrorPlaceholder?.textAlignment = self.textAlignment
        labelErrorPlaceholder?.textColor = errorTextColor
        labelErrorPlaceholder?.font = UIFont.init(name: (self.font?.fontName ?? "Roboto-Medium")!, size: 11)
        ...
        let leadingConstraint = NSLayoutConstraint.init(item: labelErrorPlaceholder!, attribute: .leading, relatedBy: .greaterThanOrEqual, toItem: self, attribute: .leading, multiplier: 1, constant: 0)
        ...
        errorLabelHieght = NSLayoutConstraint.init(item: labelErrorPlaceholder!, attribute: .height, relatedBy: .greaterThanOrEqual, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: 0)
    ...
    func hideErrorPlaceHolder(){
        ...
        self.disableFloatingLabel = false
        self.labelErrorPlaceholder?.text = ""
        errorLabelHieght?.constant = 0;
        UIView.animate(withDuration: 0.3, delay: 0, options: .curveEaseInOut, animations: {
            self.layoutIfNeeded()
        }, completion: nil)
    ...
    func floatPlaceHolder(selected:Bool) -> Void {
        ...
        labelPlaceholder?.isHidden = showingError ? true : false
        if selected {
           ...
           // self.setValue(self.selectedPlaceHolderColor, forKeyPath: "_placeholderLabel.textColor")
            ...
            //self.setValue(selectedPlaceHolderColor, forKeyPath: "_placeholderLabel.textColor")
        }
        ...
        if placeholderLabelHeight?.constant == 18 {
            return
        }
        
        placeholderLabelHeight?.constant = 18;
        labelPlaceholder?.font = UIFont.init(name: (self.font?.fontName ?? "Roboto-Medium")!, size: 15)
